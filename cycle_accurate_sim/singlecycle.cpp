...
bitset<32> signExtend(const bitset<16>& imm) {
    return bitset<32>(static_cast<int32_t>(static_cast<int16_t>(imm.to_ulong())));
}
...
int main() {
    ...
    while (!halt) {
        // Fetch
        bitset<32> instruction = instMem.read(PC);
        bitset<6> opCode = instruction.to_string().substr(26, 6);
        bitset<6> funct = instruction.to_string().substr(0, 6);
        bitset<5> rs = instruction.to_string().substr(21, 5);
        bitset<5> rt = instruction.to_string().substr(16, 5);
        bitset<5> rd = instruction.to_string().substr(11, 5);
        bitset<16> imm = instruction.to_string().substr(0, 16);

        // Decode
        controlUnit(opCode, RegDst, BranchEqual, MemReadEn, MemtoReg, ALUOp, MemWriteEn, RegWriteEn, ALUSrc, Jump, BranchNotEqual);
        ALUcontrol(ALUOp, funct, JumpReg, opSel);
